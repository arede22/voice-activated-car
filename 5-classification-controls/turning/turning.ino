...
float delta_reference(int k) {
  ...
  if (drive_mode == DRIVE_RIGHT) {
    return (TURN_RADIUS+CAR_WIDTH/2)*k*v_star/(TURN_RADIUS*5) - (TURN_RADIUS-CAR_WIDTH/2)*k*v_star/(TURN_RADIUS*5);
  }
...
float straight_correction(int k) {
  // YOUR CODE HERE
  return (STRAIGHT_RADIUS+CAR_WIDTH/2)*k*v_star/(STRAIGHT_RADIUS*5) - (STRAIGHT_RADIUS-CAR_WIDTH/2)*k*v_star/(STRAIGHT_RADIUS*5);
}
